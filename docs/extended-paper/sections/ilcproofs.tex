...
\begin{grammar}
    ...
    & $\ChTy$
    &$\bnfas$& $\emptyctxt \bnfalt \ChTy,d: S$
\end{grammar}
    ...
    \item[] \textbf{Case}
      ...
      \begin{llproof}
        ...
        \Pf{}{}{\qquad e = E[\eNu{(x_1, x_2)}{e_1}],~e'= E[
            [\eChan{\mathsf{Read}(d)}/x_1][\mathsf{Write}(d)/x_2]e_1 ],~d \not \in
          \Names}{Suppose}
        
        \Pf{}{}{\qquad \Config{\Names}{\Store}{\Procs,p:[\eNu{(x_1, x_2)}{e_1}]} ->
          \Config{\Names, d}{\Store}{\Procs, \ProcNm{p} \proc{E[
                [\mathsf{Read}(d)/x_1][\mathsf{Write}(d)/x_2]e_1 ]}}}{By rule nu}
...
\begin{lemma}[Substitution (Read Endpoint)]\label{lem:sub-rd}
  ...
  \item $\ChTy,d:S; \Delta ; \Gamma$ is defined,
  \end{enumerate}
  then $\ChTy,d:S; \Delta; \Gamma |- [\mathsf{Read}(d)/x]e : U$.
  \begin{proof}
...
\begin{lemma}[Substitution (Write Endpoint)]\label{lem:sub-wr}
  ...
  \item $\ChTy,d:S,\ChTy ; \Delta ; \Gamma$ is defined,
  \end{enumerate}
  then $\ChTy,d:S; \Delta; \Gamma |- [\mathsf{Write}(d)/x]e : U$.
  \begin{proof}
  ...
  \item[] \textbf{Case}
    \begin{mathpar}
      ...
      { \Config{\Names}{\Store}{\Procs, \ProcNm{p} \proc{E[ \eNu{(x_1, x_2)}{e} ]}} --->
        \Config{\Names, d}{\Store}{\Procs, \ProcNm{p} \proc{E[ [\mathsf{Read}(d)/x_1][\mathsf{Write}(d)/x_2]e ]}}}
    \end{mathpar}
    \begin{llproof}
...
%        \tyWr{S}~\textrm{and}~\ChTy(c_1) = \tyRd{S}}{Given}
      ...
      \Pf{d : S}{|-}{\mathsf{Read}(d) : \tyRd{S}}{By rule rdend}
      \Pf{d : S}{|-}{\mathsf{Write}(d) : \tyRd{S}}{By rule wrend}            

%      \Pf{c_1 : \tyRd{S}}{|-}{c_1 : \tyRd{S}}{By $c_2 \leadsto c_1$}
%
%      \Pf{c_2 : \tyWr{S}}{|-}{c_2 : \tyWr{S}}{By $c_2 \leadsto c_1$}

      \Pf{\ChTy_3}{|-}{[\mathsf{Read}(d)/x_1][\mathsf{Write}(d)/x_2]e :
        U~\textrm{where}~\ChTy_3=\ChTy_2,d:S}{By
        Lemmas~\ref{lem:sub-rd} and~\ref{lem:sub-wr}}

      \Pf{\ChTy_1,\ChTy_3}{|-}{\proc{E[ [\mathsf{Read}(d)/x_1][\mathsf{Write}(d)/x_2]e ]}
        : U_p}{By Lemma~\ref{lem:replacement}}

      \Pf{\ChTy,\ChTy_4}{|-}{\proc{E[ [\mathsf{Read}(d)/x_1][\mathsf{Write}(d)/x_2]e ]} :
      ...
      \Pf{\ChTy,\ChTy_4}{|-}{\Config{\Names}{\Store}{\Procs, \ProcNm{p} \proc{E[
              [\mathsf{Read}(d)/x_1][\mathsf{Write}(d)/x_2]e ]}} : (\PrTy_{\pi}, p : U)}{By
        rule cons}

      \Pf{\ChTy,\ChTy_4}{|-}{\Config{\Names}{\Store}{\Procs, \ProcNm{p}
          \proc{E[ [\mathsf{Read}(d)/x_1][\mathsf{Write}(d)/x_2]e ]}} : \PrTy}{Above}      

      \Pf{}{}{\ChTy' = \ChTy,\ChTy_4~\textrm{and}~\PrTy' = \PrTy}{Suppose}

      \Pf{\ChTy'}{|-}{\Config{\Names}{\Store}{\Procs, \ProcNm{p}
          \proc{E[ [\mathsf{Read}(d)/x_1][\mathsf{Write}(d)/x_2]e ]}} : \PrTy'}{By above equalities}      
    \end{llproof}    
