...
\begin{mathpar}
  \Infer{\st{emulate}}
        {\forall~\mc{Z}.~ 
         \mathsf{execUC}\ \mc{Z}\ \pi\ \mc{F}_1\ {\mathds{1}}_\mc{A} \le
...
$\mathds{1}_\mc{A}$ is the dummy adversary, which just relays messages between
...
can do, the environment can achieve using $\mathds{1}_\mc{A}$.
...
\begin{mathpar}
  \Infer{{emulate}}
        {\forall~\mc{Z}.~ 
         \mathsf{execUC}\ \mc{Z}\ \phi\ \mc{F}_2\ \mathds{1}_\mc{A} \le
...
%  if $\keyword(\mc{Z}, \pi, \mc{F}_0, \mc{A}_{\mathds{1}})$, then
%  $|- \keyword{polyUC}(\mc{Z}, \pi_{\mathds{1}}, \mc{F}_1, \mc{S})$, and the
...
S_{\mc{I,\pi}}$) in the natural way as well (see the appendix).
Our proof obligation is to introduce an arbitrary environment $\mc{Z}$ and conclude
\[  \keyword{execUC}~\mc{Z} ~(\rho \circ \pi)~\mc{F}_1 ~\mathds{1}_\mc{A}
\le \keyword{execUC}~\mc{Z} ~\mathds{1}_\mc{\pi} ~\mc{F}_3 ~(\mc{S}_{\mc{I},\rho} \circ \mc{S}_{\mc{I},\pi})
.\]
\noindent
%
...
\begin{align*}
    &~ \keyword{execUC}~\mc{Z}~(\rho \circ \pi)~\mc{F}_1~ \mathds{1}_\mc{A} & \\
  \equiv &~ \keyword{execUC}~(\mc{Z}\circ \rho)~\pi~\mc{F}_1~ \mathds{1}_\mc{A} &
...
\le&~ \keyword{execUC}~(\mc{Z}\circ \rho)~\mathsf{id}_\pi ~\mc{F}_2~ \mc{S}_{\mc{I},\pi} & (\textnormal{From}~ \mc{F}_1 \yrightarrow{$\pi$} \mc{F}_2) \\
 \equiv &~ \keyword{execUC}~(\mc{S}_{\mc{I},\pi}\circ \mc{Z})~\rho ~\mc{F}_2~ \mathds{1}_\mc{A} &  \textnormal{(By equivalence)} \\
...
\begin{equation}
   \keyword{execUC}~\mc{Z}~(\rho \circ \pi)~\mc{F}_1~ \mathds{1}_\mc{A} =
   \keyword{execUC}~(\mc{Z}\circ \rho)~\pi~\mc{F}_1~ \mathds{1}_\mc{A}
\end{equation}
...
\begin{equation}
  \keyword{execUC}~(\mc{Z}\circ \rho)~\pi~\mc{F}_1 ~ \mathds{1}_\mc{A} \le
...
\begin{equation}
  \keyword{execUC}~(\mc{Z}\circ \rho)~\mathsf{id}_\pi~\mc{F}_2 ~ \mc{S}_{\mc{I},\pi} =
  \keyword{execUC}~(\mc{S}_{\mc{I},\pi}\circ \mc{Z})~\rho ~\mc{F}_2~ \mathds{1}_\mc{A}
\end{equation}
...
given in the appendix, is trivial, but necessary because our protocol emulation definition requires simulation in both directions.
