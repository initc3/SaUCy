\begin{figure*}
  ...
  \begin{mathpar}
  \Infer{kgen}
  {\keyword{\textbf{Gen}}(v_n, v_r) = (v_{pk}, v_{td}) \\ v_{k}, v_{td} \in \{0,1\}^{v_n}}
  { \Store ; \eKGen{v_n}{v_r} ---> \Store ; (v_{pk}, v_{td})}
  \and
  \Infer{tdp}
  ...
  \and
  \Infer{inv}
  {\mathbf{Inv}((v_{pk}, v_{td}), v_{in}) = v_{out} \\ \mathbf{Inv} \colon \{0,1\}^n \times
    \{0,1\}^n -> \{0,1\}^n -> \{0,1\}^n}
