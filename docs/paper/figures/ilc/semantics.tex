...
\begin{grammar}
  Channel names & $c$   &$\bnfas$& $\textrm{Read}(d) ~|~ \textrm{Write}(d)$
  \\
  Inner channel names & $d$ &$\bnfas$& $\cdots$
  \\  
...
\end{grammar}
\end{subfigure}
\caption{Channel names, inner channel names, process names, configurations, and evaluation contexts.}
\label{fig:configs}
\end{figure*}
