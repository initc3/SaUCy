...
normalization, so we define this additionally.

\begin{definition}[Polynomial time term]
  A term \[\emptyctxt |- \mathsf{f} : [\tyBit]\ {->}_m\ A\] is polynomial time if for all
  random bitstreams \textsf{r : [\tyBit]}, where the length of \textsf{r} is a
  polynomial $L(k)$ in the security parameter $k$, applying \textsf{f} to
  \textsf{r} normalizes to a value \textsf{v}. Furthermore, this normalization
  takes a polynomial $\varphi(k)$ number of steps.
\end{definition}

Because processes are confluent, we know that there is only one such value
\textsf{v = f r} for each bitstream \textsf{r} and that \textsf{f} is a
polynomial time computable function. Hence, a uniform distribution of bitstrings
gives us a distribution of \textsf{v}: \todo{}.
