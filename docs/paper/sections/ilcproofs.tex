...
\begin{theorem}[Local Progress]
  For all expressions $e$ and types $U$, if $|- e : U$, then either $\Lterm{e}$
...
\begin{theorem}[Local Preservation]
  For all expressions $e, e'$ and types $U$, if $|- e : U$ and $e -> e'$, then
  $|- e' : U$.
  \begin{proof}
...
\end{mathpar}
\begin{align*}
  \textrm{RdChans}(\emptyctxt) &= \emptyctxt
  &\textrm{WrChans}(\emptyctxt) &= \emptyctxt\\
...
\begin{lemma}[Substitution (Intuitionistic)]
  If $\Delta; \Gamma, x : A |- e : A'$ and $\Delta; \Gamma |- e : A$, then $\Delta; \Gamma |- [e/x]e' : A'$.
  \begin{proof}
    By structural induction on the derivation of $\Delta; \Gamma, x : A |- e : A'$.
  \end{proof}
\end{lemma}

\begin{lemma}[Substitution (Affine)]
  If $\Delta, x:X; \Gamma |- e : X'$ and $\Delta; \Gamma |- e : X$, then $\Delta; \Gamma |- [e/x]e' : X'$.
  \begin{proof}
    By structural induction on the derivation of $\Delta, x ; X; \Gamma |- e : X'$.
  \end{proof}
\end{lemma}
