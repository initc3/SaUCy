...
Some careful programming is needed to handle the cases
when the adversary sends messages on behalf of corrupted parties; this is 
captured by \textsf{wrapper\{P,Q\}} (shown in the Appendix \todo{}).
...
To briefly summarize what is going: the setup CRS functionality (given in the Appendix) samples a random string $\sigma$ and two trapdoor pseudorandom generators (prgs).
...
The ideal world simulator is ported directly from the UC literature~\cite{canetti2001commitments}, while the non-standard real world simulator, given in the Appendix, is required because our protocol emulation definition requires simulation in both directions.
