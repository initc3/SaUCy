...
\label{subsec:saucy}
\label{subsec:uc}
We implement the UC execution model (roughly) as follows:

\lstinputlisting[style=myilc]{listings/simp-suc.ilc}

\noindent Before we explain what is going on here, note several things:
...
The function \textsf{execUC} is parameterized by an environment
\textsf{z}, protocol parties \textsf{p} and \textsf{q}, an adversary \textsf{a},
an ideal functionality \textsf{f}, a security parameter \textsf{k}, a random
bitstring \textsf{r}, and a static corruption model \textsf{crupt}. 

The function splits a random bitstring and distributes them to each of the
parties when they are run. Note that each protocol party are run in a
\textsf{wrapper} function, which determines its behavior based on the static
corruptions. If a party is corrupted, then the adversary can masquerade as the
party.
...
\lstinputlisting[style=myilc]{listings/non-reentrant.ilc}
