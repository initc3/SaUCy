...
\begin{definition}[Polynomial time in UC]
  ...
  \begin{mathpar}
    \Infer{polyUC}
    {\keyword{poly}(\keyword{execUC}~\mc{Z}~\pi~\mc{F}~\mc{A})}
    {\keyword{polyUC}(\mc{Z}, \pi, \mc{F}, \mc{A})}
  \end{mathpar}
  ...
  are all polynomial time in UC. \todo{Explain better?}
\end{definition}
...
\begin{definition}[Emulation]
  ...
  (written $(\pi_0, \mc{F}_0) \approx (\pi_1, \mc{F}_1)$)
  if for all adversaries $\mc{A}$, there exists a simulator $\mc{S}$ such that
  for all environments $\mc{Z}$, 
  if $\keyword{polyUC}(\mc{Z}, \pi_0, \mc{F}_0, \mc{A})$, then
  $\keyword{polyUC}(\mc{Z}, \pi_1, \mc{F}_1, \mc{S})$, and
  \[ D(\mathsf{execUC}\ \mc{Z}\ \pi_0\ \mc{F}_0\ \mc{A}) \approx D(\mathsf{execUC}\ \mc{Z}\ \pi_1\ \mc{F}_1\ \mc{S}).\]

  \todo{Use judgement form?}
  \begin{mathpar}
    \Infer{emulate}
    {\forall~\mc{A}.~\exists~\mc{S}.~\forall~\mc{Z}.~\keyword{polyUC}(\mc{Z},\pi_0,\mc{F}_0,\mc{A})
      =>\\\\
     (\keyword{polyUC}(\mc{Z},\pi_1,\mc{F}_1,\mc{S}) \land\\\\
      D(\mathsf{execUC}\ \mc{Z}\ \pi_0\ \mc{F}_0\ \mc{A}) \approx
      D(\mathsf{execUC}\ \mc{Z}\ \pi_1\ \mc{F}_1\ \mc{S}))}
    {(\pi_0, \mc{F}_0) \approx (\pi_1, \mc{F}_1)}
  \end{mathpar}
\end{definition}
