...
\end{wrapfigure}
\todo{``Too short to be useful'' needs more intuition.} The implementation of
SaUCy is centered around a definition of UC execution model in ILC.  For the
most part, this just involves connecting channels as illustrated in
Figure~\ref{fig:execUC-diagram}. For demonstration, we only show the case of
...
Still, the observation of Camenisch~\etal~\cite{camenisch2016universal} is that
neither of the above two solutions are completely satisfactory. The reentrancy
problem arises mainly due to the exchange of metamessages between processes, and
so the most compelling solution is to restrict how the environment/adversary
respond to these metamessages. While we leave modeling this solution as future
work, ILC provides the ideal starting point---restrictions on metamessages could
...
message on B. \todo{Check, need more explanation of problem earlier}
