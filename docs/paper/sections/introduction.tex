...
\emph{exactly one process is active at any given time}, and, moreover, \emph{the
...
Where do existing process calculi fall short? On the one hand, process calculi
such as the $\pi$-calculus~\cite{milner1999communicating} or its
...
non-confluent reductions by design (i.e., non-probabilistic nondeterminism).  On
the other hand, various other calculi that do enjoy confluence are restrictive,
for example, only allowing for fixed or two-party
...
virtue of where they perform read and write effects: When process $A$ writes to
process $B$, process $A$ ``spends'' the write token and process $B$ acquires
it. Moreover, to maintain that the order of activations is fully determined, the
read endpoints of a channel are an \emph{affine} resource, so each write
operation corresponds to a single, unique read operation. Together, these give
ILC its central metatheoretic property of \emph{confluence}. The only
nondeterminism in an ILC program is due to random coin tosses taken by
processes, which have a well-defined distribution. Additionally, any apparent
