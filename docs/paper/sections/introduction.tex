...
the purpose of systematizing UC, which (importantly) admits computational
security proofs, and the concretization of the UC framework itself.

\subsection{Interactive Lambda Calculus}

So where have existing concurrency models fallen short?  It turns out that
...
and hence, do not have computational interpretations.  Therefore, to faithfully
capture the computational model underlying UC, ILC adapts ITMs to a subset of
the $\pi$-calculus through its type system. In other words, \emph{well-typed
  programs are expressible as ITMs}.

To give a quick tour of ILC's type system, typing judgements have the form $\Delta ;
\Gamma |- e : A |> m$, where $\Delta$ is a linear typing context and $m \in \{\Wm, \Rm,
\Vm\}$ is a mode (write, read, and value, respectively). It maintains the
...
due to our restriction on parallel write mode composition, and as a consequence,
the mode of higher order functions can be dependent on the modes of its function
arguments.
