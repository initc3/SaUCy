...
ITMs by adapting them to a subset of the $\pi$-calculus through an affine type
...
Why do we need to design another process calculus in the first place? Where do
existing ones fall short? On the one hand, process calculi such as the
...
process $B$, process $A$ ``spends'' the write token and process $B$ ``earns''
the write token. Moreover, to maintain that the order of activations is fully
determined, the read endpoints of a channel are an \emph{affine} resource, so
each write operation corresponds to a single, unique read operation. Together,
these give ILC its central metatheoretic property of
\emph{confluence}. Consequently, the only nondeterminism in an ILC program is
due to random coin tosses taken by processes, which have a well-defined
