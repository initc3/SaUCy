\begin{abstract}
  \todo{Abstract needs revising.} The universal composability (UC) framework is
  an established gold standard for analyzing cryptographic protocols in a
  modular way from simpler building blocks. In the framework, security is
  preserved under \emph{concurrent} composition with arbitrary other protocols,
  so a UC-secure protocol remains so when dropped into any context.
