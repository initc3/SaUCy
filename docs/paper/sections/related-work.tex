...
%Since our aim is to express cryptographic theory,  does does not aim to show computational
%proofs. Instead, they prove security relative to idealized notions
%of cryptography primitives. Such 

%Importantly, they introduced
...
\label{subsec:tools}
There are a variety of computer-aided tools for cryptographic analysis, some of
which operate in the symbolic model of cryptography while others operate in the
computational model of cryptography. The survey by
Blanchet~\cite{blanchet2012security} details the differences between these two
approaches.

In the symbolic model, cryptographic operations are abstracted as term algebras,
and adversary capabilities are defined by nondeterministic applications of
deduction rules over these terms. This abstract view of protocols leads to a
high degree of automation, but weaker guarantees compared to analysis done in
the computational model. Among the symbolic tools include the NRL protocol
analyzer~\cite{meadows1996nrl}, Maude-NPA~\cite{escobar2009maude}, and
Proverif~\cite{blanchet2010proverif}.

In the computational model, \todo{\ldots} Among the computational tools include
...
$\text{F}^{*}$~\cite{swamy2016dependent}. The focus of these tools is on
game-based security proofs, which, in contrast to simulation-based security
definitions (such as UC), only guarantee security in a standalone
setting. Although the computational tools do not support message-passing
concurrency, it would be interesting to embed ILC into EasyCrypt or F* in order
to use its tooling.
...
and give a concrete programming model.
...
We follow this in our \textsf{execUC} definition.
