...
\label{subsec:tools}
Computer-aided tools for cryptographic analysis operate in either the symbolic
...
%and adversary capabilities are nondeterministic applications of deduction rules
%over these terms. This abstract view of protocols leads to a high degree of
%automation, but weaker security guarantees than in the computational
%model. Symbolic tools include the NRL protocol analyzer~\cite{meadows1996nrl},
%Maude-NPA~\cite{escobar2009maude}, and Proverif~\cite{blanchet2010proverif}.
...
Computational tools include CertiCrypt~\cite{barthe2009formal},
...
%tools do not support message-passing concurrency, it would be interesting to
%embed ILC into EasyCrypt or F* in order to use its tooling.
...
A number of models for universal composability have been proposed in the
...
Models based on inexhaustible interactive Turing machines
...
%Our work is orthogonal, but comes with the benefit of
%flexibility. For example, our type system subsumes their notion of
%\emph{compatibility} and \emph{connectability}, we avoid the distinctions of
%\emph{net}, \emph{external}, \emph{io}, etc. tapes, and we give a concrete
%programming model. \todo{Rewrite.}
