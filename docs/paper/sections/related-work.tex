...
We first touch on two distinct but useful approaches for analyzing cryptographic
...
these terms~\cite{cortier2011survey}. Importantly, adversaries make
nondeterministic choices, and so an attack trace will be found if there is
one. On the upside, Viewing protocols from this abstract vantage point makes the
analysis relatively tractable. However, because adversaries have
nondeterministic behavior, their capabilities must be severely
restricted. Therefore, it is possible to verify protocols that are susceptible
to simple attacks that lie outside the adversary model.
...
(p.p.t.)  Turing machines. Importantly, security is defined in terms of the
probability and computational complexity of attacks, which complicates proofs,
but admits much stronger security guarantees. Analyzing protocols in the
computational model is the modus operandi in cryptography, but, for the most
part, proofs are done manually. The goal of this work, and others we discuss
later, seek to change this dynamic.

There are also a number of works on computational soundness, which focuses on
bridging the gap between the symbolic and computational
...
Cryptography and formal methods have a flourishing interplay, and there are
variety of computer-aided tools for cryptographic analysis, both symbolic and
computational. Among the symbolic tools include the NRL protocol
