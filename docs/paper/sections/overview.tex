...
arguments. (When the context is clear, we refer to ``channel endpoints'' as
simply ``channels.'') The first is a write channel \textsf{toQ : Wr Msg} (for
sending messages of type \textsf{Msg} to $Q$), and the second is a read channel
...
contexts $\Delta_1; \Gamma$ and $e_2$ evaluates to a write channel (of type
...
\Delta_2$ such that $e_1$ evaluates to a read channel (of type $\tyRd{S}$) under
