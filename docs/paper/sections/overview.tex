...
\textsf{fCom}, which (glossing over some notation) takes as arguments two
...
second process, so the mode of $\eFork{e_1}{e_2}$ is $m_1 || m_2 => m_3$.
...
\begin{itemize}[leftmargin=*]
  ...
  \item Because ILC only allows intuitionistically typed values to be sent over
    channels, the value read on the channel is lifted into the first element of
    the tensor as a linear type.
...
\footnote{Note that in the real world, all communication
