...
channel \textsf{toQ : Wr Msg} (for writing messages to $Q$) and a read channel
\textsf{frP : Rd Msg} (for reading messages from $P$). It should be fairly
straightforward to see how the communication pattern in \textsf{fCom} follows
that in $\Func_{\textsc{com}}$.  There are a few details that require some
explanation, so for these, we give a taste of the type system.

\subsection{ILC Type System}
\label{subsec:type-tour}

ILC terms are either intuitionistically typed, meaning they can be freely
copied, or affinely typed, meaning they can be used at most once. Here, affine
typing serves a special purpose: to ensure that \emph{well-typed ILC programs
  are expressible in ITMs}. To make this happen, the typing rules maintain these
invariants:

\begin{itemize}[leftmargin=*]
\item \emph{Only one process is ``active'' at any given time.} Processes
  implicitly pass around an affine ``write token'' $\wrtok$ by virtue of where
  they perform read and write effects. Because the write token is affine
  (unique), at most one process owns the write token (is ``active'') at any
  given time.
  
\item \emph{The sequence of activations is deterministic.} Each channel (or
  ...
  there is a unique process at the receiving end of a communication.
\end{itemize}

To showcase these invariants, we give a taste of the typing rules, namely, those
for forking a process (fork), reading a message (rd), and writing a message
(wr). We read the typing judgement $\Delta ; \Gamma |- e : U$ as ``under affine context
$\Delta$ and intuitionistic context $\Gamma$, expression $e$ has type $U$.''
\begin{mathpar}
\Infer{fork}
{\Delta_1; \Gamma |- e_1 : U\\\\
\Delta_2; \Gamma |- e_2 : V}
...
The fork rule says that if we can partition the affine context as $\Delta_1, \Delta_2$
such that $e_1$ has type $U$ under contexts $\Delta_1; \Gamma$, and $e_2$ has type $V$
under contexts $\Delta_2; \Gamma$, then the expression has type $V$. Here, $e_1$ is the
child process, and $e_2$ is the parent process. Notice that affine resources
must be partitioned between the two processes, and so each read channel and the
write token is owned by at most one process.
\begin{mathpar}
\Infer{wr}
{\Delta_1; \Gamma |- e_1 : S\\\\
...
The wr rule says that if we own the write token and we can partition the affine
context as $\Delta_1, \Delta_2$ such that $e_1$ has type $S$ under contexts $\Delta_1; \Gamma$, and
$e_2$ has a type $\tyWr{S}$ under contexts $\Delta_2; \Gamma$, then the \textsf{wr}
expression has type $\tyUnit$. Here, $S$ ranges over the types of values that
are ``sendable.'' Notice that typing a write expression ``spends'' the write
token.
\begin{mathpar}
\Infer{rd}
{\wrtok \not \in \Delta_1,\Delta_2\\
\Delta_1 ; \Gamma |- e_1 : \tyRd{S}\\\\
\Delta_2,\wrtok, x : \tyTensor{\tyBang{S}}{\tyRd{S}} ; \Gamma |- e_2 : U}
{\Delta_1,\Delta_2 ; \Gamma |- \eLetRd{e_1}{x}{e_2} : U}  
\end{mathpar}

The rd rule says that if we do not own the write token, and we can partition the
affine context as $\Delta_1, \Delta_2$ such that $e_1$ has type $\tyRd{S}$ under contexts
$\Delta_1; \Gamma$, and $e_2$ has type $U$ under contexts $\Delta_2, \wrtok,
x:\tyTensor{\tyBang{S}}{{\tyRd{S}}} ; \Gamma$, then the \textsf{rd} expression has
type $U$. Here, an affine pair is bound to the variable $x$, with the (sendable
and intuitionistic) value read over the channel (wrapped in a $!$ operator to
make it affine) as the first coordinate and the read
channel as the second coordinate (so it can be reused). Notice, that typing a
read expression allows the write token to be spent in $e_2$.
