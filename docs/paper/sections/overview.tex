...
The write expression $\eWr{e_1}{e_2}$ sends the result of evaluating $e_1$ on the
write channel result of evaluating $e_2$. One thing to mention is that only sendable-typed
...
result of evaluating $e_1$ and binds the value-channel pair as $x$ in the affine
...
intuitionistic arrows) and \textsf{frP : Rd Msg} is affine ($\multimap$ is the
type connective for affine arrows) as expected. To see that \textsf{frP} is
being used affinely, notice that it is rebound when deconstructing the
value-channel pair from each read operation. To see that the write token is
being passed around appropriately, notice that the read and write effects are
interleaved.
