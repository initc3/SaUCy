...
can take place there does not lead to a break in security in the real world.
...
right, and connecting lines as communication channels. There are several things
worth mentioning:
...
\item In the ideal world, parties $A_{\mathbbm{1}}$ and $B_{\mathbbm{1}}$ are
  ``dummy'' parties that simply relay information between the environment
  $\mc{Z}$ and the ideal functionality $\mc{F}$.
\item The environment $\mc{Z}$ can interact with each of the worlds in the same
  way, with the main difference being that, in the ideal world, it is
  (indirectly) interacting with the ideal functionality.
\end{itemize}
