...
serves a special purpose, namely, to ensure that ILC processes have a determined
sequence of activations, as is required in ITMs. This is achieved through the
...
might justifiably wonder why write channels are intuitionistic and read channels
...
case, this does not typecheck, since read channels are affine
resources. Together, these invariants ensure that processes have a determined
...
The type signature tells us that \textsf{toQ : Wr Msg} is intuitionistic ($->$
is the type connective for intuitionistic arrows) and \textsf{frP : Rd Msg} is
...
preventing write nondeterminism; read channels \emph{are} affine, which helps
