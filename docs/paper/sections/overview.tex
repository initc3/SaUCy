...
\begin{subfigure}{.575\textwidth}
    $\Func_{\textsc{com}}$ proceeds as follows, running with committer $P$ and receiver
  $Q$.
    \begin{enumerate}
...
\item For simplicity, protocols have two parties: $A$ and $B$ in the real world,
  and $A_{\mathds{1}}$ and $B_{\mathds{1}}$ in the ideal world.
...
\item In the ideal world, parties $A_{\mathds{1}}$ and $B_{\mathds{1}}$ are
...
$A_{\mathds{1}}$ and $B_{\mathds{1}}$ are ``dummy'' parties that simply relay
...
In Figure~\ref{func:com} (right), we implement a simplified version of
...
The fork expression $\eFork{e_1}{e_2}$ spawns a child process $e_1$ and continues
as $e_2$.
\begin{mathpar}
\Infer{fork}
{\Delta_1; \Gamma |- e_1 : U\\
...
values of a sendable type (ranged over by $S$) can be sent over channels (more
on this later).
\begin{mathpar}
\Infer{wr}
{\Delta_1; \Gamma |- e_1 : S\\
...
\textsf{fCom} respects the invariants of the type system.
