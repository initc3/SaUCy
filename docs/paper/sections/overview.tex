...
Let us first examine the function signature of \textsf{fCom}, which takes as
arguments two channels, a write channel \textsf{toQ} (writes values of type
\textsf{Msg} to $Q$) and a read channel \textsf{frP} (reads messages of type
\textsf{Msg} from $P$) , and returns \textsf{Unit}. There are a number of things
to unpack here:

\begin{itemize}[leftmargin=*]
  \item The signature consists of a combination of intuitionistic arrows $->$
    and affine arrows $\multimap$ (or ``lollipops''). Lollipops describe the types of
    functions that consume their argument at most once.
  \item Both kinds of arrows carry the \emph{modal type} (or \emph{mode}) of
    their function bodies. A mode can have one of three values: either write
    $\Wm$, read $\Rm$, or value $\Vm$.  In the example, the left lollipop has
    mode $\Vm$, which we elide, and the right lollipop has mode $\Rm$. We have
    more to say about modes later.
  \item Read channels are affinely typed to protect them from duplication.
  \item Write channels and the unit value are intuitionistically typed.
\end{itemize}
...
$\eSeq{e_1}{e_2}$ (syntactic sugar for $\eLet{}{\eUnit}{e_1}{e_2}$) is derived
as $m_1 ;; m_2 => m_3$. Parallel mode composition shows in the case of forking a
child process, so the mode of $\eFork{e_1}{e_2}$ (fork a process $e_1$ and
continue as $e_2$) is derived as $m_1 || m_2 => m_3$. The rules for mode
composition are discussed in Section~\ref{sec:ilc}.
...
process is currently writing, or equivalently, which process is currently
active.\smallskip
...
form $\Delta ; \Gamma |- e : U |> m$, where $\Delta$ is an affine typing context, $\Gamma$ is an
...
$\Delta_1; \Gamma$, and $e_2$ has type $U$ and mode $m$ under contexts
$\Delta_2,x_1:\tyBang{A},x_2:\tyRd{A} ; \Gamma$, then the full expression has type $U$ and
...
Revisiting \textsf{fCom}, by rule letrd, the mode of the entire function body is
$\Rm$. This explains why the lollipop in the function signature carries the mode
$\Rm$.
