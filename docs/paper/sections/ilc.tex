...
include unit ($\tyUnit$), products ($\tyProdd{}{S}{T}$),
sums ($\tySum{S}{}{T}$).


The standard intuitionistic types include unit ($\tyUnit$), products
($\tyProdd{}{A}{B}$), sums ($\tySum{A}{}{B}$), and arrows ($\tyArr{A}{\infty}{U}$ or
simply $A -> U$). These types, along with a special ``write'' arrow type
($\tyArr{A}{\wm}{U}$), round out the sendable types, i.e., the types of values
that can be sent over channels. \todo{Why?} The remaining intuitionistic type is
