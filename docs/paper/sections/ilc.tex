...
($\tyLolli{X}{\footnotesize m}{Y}$). Notice that only intuitionistically typed
values can be wrapped in a bang! or sent over a channel.
...
\todo{Discuss store, process pool, and channel typings.}

The expressions for variables ($x$), unit, products, sums, references, intuitionistic
fixed points, intuitionistic functions, and intuitionistic let binding are
standard. Expressions related to affinity are more interesting: Lollipops are
introduced by $\eLAM{x}{e}$ and eliminated by
\todo{}, bang! types are introduced by $\eBang{e}$ and eliminated by
$\eLetBang{x}{e_1}{e_2}$, and the expressions for message-passing and
...
\paragraph{Affine rules.}

\subsection{Dynamic Semantics}
\label{subsec:semantics}

\todo{Need some help explaining this subsection.} Figures~\ref{fig:configs} and~\ref{fig:semantics} define the dynamic semantics
