...
expressions. To streamline the syntax, certain elements are parameterized by a
multiplicity $\pi$ to distinguish between the intuitionistic ($\infty$) and affine
($1$) counterparts. Moreover, certain elements are parameterized by a function
label $\ell$, which includes the multiplicity labels along with an additional label
$\wm$ to be explained later.

Types in ILC (written $U$, $V$) are bifurcated into intuitionistic types
(written $A$, $B$) and affine types (written $X$, $Y$).  The standard
intuitionistic types include unit ($\tyUnit$), products ($\tyProdd{\infty}{A}{B}$ or
simply $\tyProdd{}{A}{B}$), sums ($\tySum{A}{\infty}{B}$ or simply $\tySum{A}{}{B}$),
and arrows ($\tyArr{A}{\infty}{U}$ or simply $A -> U$). These types, along with a
special ``write'' arrow type ($\tyArr{A}{\wm}{U}$ or simply $A \twoheadrightarrow U$), round out
the sendable types, i.e., the types of values that can be sent over
channels. The remaining intuitionistic type is the write channel type
($\tyWr{S}$), whose constructor can take sendable types, but does not yield a
sendable type.

Affine types include bang types ($\tyBang{A}$), affine pairs or ``tensors''
($\tyProdd{1}{X}{Y}$ or simply $\tyTensor{X}{Y}$), affine sums
($\tySum{X}{1}{Y}$ or simply $\tyOplus{X}{Y}$), affine arrows or ``lollipops''
($\tyArr{X}{1}{U}$ or simply $X \multimap U$), and read channel types ($\tyRd{S}$).
...
interesting expressions are those related to affinity and message-passing
concurrency:
\begin{itemize}[leftmargin=*]
  ...
  \item \emph{Read:} $\eRd{e}$ reads a value from the read channel result of
    evaluating $e$. (There is more going on here, but it is better explained in
    the context of the typing rules.)
...
The affine write token \todo{}.

\subsection{Static Semantics}
\label{subsec:types}

The type system of ILC is given in Figure~\ref{fig:type-expressions}. The
algorithmic typing judgment $\Delta_{\emph{in}} ; \Gamma |- e : U -| \Delta_{\emph{out}}$
should be read as follows:
\begingroup
\addtolength\leftmargini{-.2in}
\begin{quote}
  Under input contexts $\Delta_{\emph{in}}$ and $\Gamma$, expression~$e$ has type $U$ and
  output context $\Delta_{\emph{out}}$.
\end{quote}
\endgroup
\noindent Importantly, the type
