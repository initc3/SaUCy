...
The metavariables $U$,$V$ range over all types, which are then bifurcated into
intuitionistic types (written $A$, $B$) and affine types (written $X$, $Y$). The
standard intuitionistic types include unit ($\tyUnit$), products
($\tyProd{A}{B}$), and sums ($\tySum{A}{B}$). The more interesting ones are
write channels ($\tyWr{A}$) and arrows ($\tyArr{A}{\footnotesize m}{U}$), which
importantly carry a mode. Affine types include read channels ($\tyRd{A}$), bang!
types ($\tyBang{A}$), tensors ($\tyTensor{X}{Y}$), affine sums ($X \oplus Y$), and
lollipops ($\tyLolli{X}{\footnotesize m}{U}$).
...
Figure~\ref{fig:mode-composition} (discussed later).

To make the syntax more terse, multiplicities (written $pi$) include $1$
(affine) and $\infty$ (intuitionistic).
...
$\eUnbang{e}$. Lollipops have introduction form $\eLam{1}{x}{e}$ and elimination
form $\eApp{e_1}{e_2}{1}$. Tensors have introduction form $\ePair{e_1}{e_2}{1}$
and elimination form $\eSplit{1}{e_1}{x_1}{x_2}{e_2}$. Affine sums have
introduction form $\eInj{1}{i}{e}$ and elimination form
$\eCase{1}{e}{x_1}{e_1}{x_2}{e_2}$. The expressions for message-passing
concurrency are these:
...
The type system of ILC is given in Figure~\ref{fig:type-expressions}. The typing judgment $\Delta ; \Gamma |- e : U |> m$ should be read as follows:
\begingroup
\addtolength\leftmargini{-.2in}
\begin{quote}
  Under affine context $\Delta$ and intuitionistic context $\Gamma$, the expression $e$
  has type $U$ and mode $m$.
\end{quote}
\endgroup
\noindent Importantly, the type system maintains these invariants:
...
avoid clutter; algorithmic typing rules are given in the Appendix.
...
intuitionistic lambda abstractions (iabs) and fixed points (ifix) stipulate that
they must be closed with respect to affine variables. Otherwise, they could
...
composition of the mode of $e_1$ and the mode of $e_2$.  To ensure this property
is preserved during normalization, we must also forbid sequential composition of
write mode processes. This prevents the following violation of parallel writes:
Suppose process $A$ has mode composition $\Rm ;; \Wm$ and process $B$ has mode
composition $\Wm ;; \Wm$. If $A$ and $B$ communicate, then both processes
continue as mode $\Wm$, leading to a race condition.
