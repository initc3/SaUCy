...
special ``write'' arrow type ($\tyArr{A}{\wm}{U}$), round out the sendable
types, i.e., the types of values that can be sent over channels. \todo{Why?} The
remaining intuitionistic type is the write channel type ($\tyWr{S}$), whose
constructor can take sendable types, but does not yield a sendable type.
