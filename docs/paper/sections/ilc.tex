...
fixed point expressions, and references. \todo{Let bindings.} Affine functions
are introduced using $\eLAM{x}{e}$ and an intuitionistically typed expression
$e$ can be lifted into an affinely typed expression as $\eBang{e}$. \todo{App
  for lollipops?}  Expressions for message passing and concurrency are the
following:
