...
syntax label $\ell$, which includes the multiplicity labels along with an
...
write arrows ($\tyArr{A}{\wm}{U}$). \todo{Why do we need sendable types?}
...
denotes the sendable variants; on functions, it denotes the special write
function.
...
ownership of the write token), and the sequence of activations is deterministic
(unique ownership of read channels).

The typing rules for the functional fragment of ILC are fairly standard. We
comment on the nonobvious rules, such as the ones for various forms of
abstraction (iabs, wabs, aabs) and application (wapp).
...
would not be able to specify a ``starting process'' function that is initially
activated.

Affine abstraction (aabs) is similar to intuitionistic abstraction, except it
takes an affine argument. The body of an affine lambda expression \emph{can}
...
variables, then it can be transformed into an intuitionistic function $g : A ->
X \multimap U$.

The typing rules for communication and concurrency were mostly covered in
