...
We omit these details here for space reasons, and we refer the interested reader to \Secref{sec:ilcproofs}.
...
%
The rules of this judgement follow a standard call-by-value semantics, 
where we streamline the definition with an evaluation context~$E$.

Configuration stepping conists of five rules, including a congruence
...
creating new proceses (via \Rule{fork}),
...
names of channels (read and write ends) and procesess in the name set.
%
Rule~\Rule{rw} uses the syntax~$R$ for a combination of several (mutually exclusive) read expressions; when synchronized, 
