...
channels. \todo{Why?} The remaining intuitionistic type is the write channel
type ($\tyWr{S}$), whose constructor can take sendable types, but does not yield
a sendable type.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \emph{Read:} $\eLetRd{e_1}{x}{e_2}$ reads a value from the read channel
    result of evaluating $e$. (There is more going on here, but it is better
    explained in the context of the typing rules.)
...
The type system of ILC is given in Figure~\ref{fig:type-expressions}.
%
We read the typing judgement $\Delta_{\emph{in}} ; \Gamma |- e : U -| \Delta_{\emph{out}}$ as
